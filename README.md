...
## Overview:
	Web scraping takes advantage of Python, Mongo, Flask, and HTML to allow us to write a script that will gather data from a designated webpage, store and render that information in a webpage that we can tailor to specific users' needs. This exercise will look at scraping data from an astrology website with information about Mars. We will utilize the Splinter module in Python that takes advantage of a headless browser, replicating a person's actions on a website. We also use the Pandas and Beautifulsoup modules to find and retrieve the information we are looking for within the page. Once we have what we need, we can store that information in a Mongo database and then take advantage of the flask functionality to pass the data to an HTML template that we create. 
